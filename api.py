...
def _problem(status: int, detail: str, kind: str, **extra: Any) -> JSONResponse:
    return JSONResponse(
        status_code=status,
        content={"detail": detail, "kind": kind, **extra},
    )
...
async def _upstream(request: Request, error: services.UpstreamError):
    ...
    return _problem(
        503,
        "An external database is unavailable right now. " + str(error),
...
async def _bad_request(request: Request, error: RequestValidationError):
    ...
    for item in error.errors():
        ...
        parts.append(
            f"{where}: {item.get('msg', 'invalid value')}"
            if where
            else str(item.get("msg", "invalid"))
        )

    return _problem(
        422,
        "; ".join(parts) or "The request could not be understood.",
        "invalid_input",
    )
...
def reference_row(code: str) -> dict[str, Any]:
    ...
    if base is None:
        raise services.UpstreamError(
            "not_found",
            f"'{code}' is not one of the 20 standard amino acids.",
        )
...
def health() -> dict[str, Any]:
    ...
    return {
        ...
        "prosite": {
            ...
            "patterns": info.get("patterns_usable"),
        },
    }
...
def status_report() -> dict[str, Any]:
    return {
        ...
        "dataset_scopes": {
            k: v["label"]
            for k, v in services.SCOPES.items()
        },
    }
...
def list_proteins(
    ...
    if page_size not in services.PAGE_SIZES:
        raise HTTPException(
            status_code=422,
            detail="page_size must be one of "
...
def protein_analysis(
    protein_id: str,
    hydropathy_window: int = Query(default=pa.DEFAULT_HYDROPATHY_WINDOW),
) -> dict[str, Any]:
    ...
    return {
        ...
        "analysis": analysis_for(record, hydropathy_window),
    }
...
def protein_structures(protein_id: str) -> dict[str, Any]:
    return structures.list_for(
        uniprot.get_protein(protein_id)
    )
...
def structure_file(
    ...
    text, fmt, meta = structures.fetch_file(
        source,
        ident,
        protein_loader=uniprot.get_protein,
    )
...
def analyze_sequence(body: AnalyzeBody) -> dict[str, Any]:
    return engine.analyze_sequence(
        body.sequence,
        body.hydropathy_window,
    )
...
def compare(body: CompareBody) -> dict[str, Any]:
    ...
    for accession, raw in (
        (body.first, body.first_sequence),
        (body.second, body.second_sequence),
    ):
        if raw and raw.strip():
            ...
            sides.append(
                (
                    protein_summary(record),
                    record["sequence"],
                )
            )

        else:
            raise engine.InvalidSequence(
                "Choose two proteins (accessions) or paste two sequences to compare."
            )

    (first, first_seq), (second, second_seq) = sides

    result = engine.compare_proteins(
        first_seq,
        second_seq,
        body.kmer_size,
    )
...
def amino_acid_detail(code: str) -> dict[str, Any]:
    ...
    return {
        "residue": row,
        "structure": references.get(row["code"]),
    }
...
def dataset_statistics(
    scope: str = "human_reviewed",
    refresh: bool = False,
) -> dict[str, Any]:

    return dataset_stats.status(
        scope,
        refresh,
    )
...
if not (FRONTEND_DIR / "index.html").is_file():
    raise RuntimeError(
        f"The 'frontend' folder with index.html was not found next to api.py "
        f"({FRONTEND_DIR}). Extract the whole project zip and keep the folder structure."
    )
